...
# capture-onedrive-state-interactive.sh
# Interactive OneDrive local storage audit with colored terminal output
# Finds all folders using more than 50KB of local storage
# Scans up to 4 levels deep to identify specific local folders
# Helps identify what's pinned as "Always keep on this device"
#
# This is the standalone interactive version with progress display and colors.
# For automated baseline snapshots, use capture-onedrive-state.sh instead.
