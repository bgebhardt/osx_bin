...
# Push a local outbox folder up to one Remotely Save S3-compatible bucket
# with rclone.
#
# This is intentionally upload-only and additive-only (rclone copy, never
# sync/bisync, no --delete-* flags anywhere in this script):
#   local outbox folder -> remote Backblaze bucket
#
# It only ever adds or overwrites objects in the bucket. It can never delete
# anything remotely, so a misconfigured run's worst failure mode is "nothing
# new shows up" rather than "existing vault content disappears." It is the
# counterpart to pull-remotely-save.sh, which is download-only in the other
# direction and lives alongside this script.
#
# Usage:
#   push-remotely-save.sh --name NAME --bucket BUCKET --source DIR [options]
#
# Required:
...
#   --source DIR      Local folder to push. Its contents are copied to the
#                     bucket root, so a file at DIR/Foo/bar.md lands at
#                     bucket path Foo/bar.md (and, once any other device's
#                     Remotely Save pulls it, at <vault>/Foo/bar.md there).
#
# Optional:
...
#
# The Backblaze application key must have write (not just read) capability
# on the bucket, or every run will fail at the rclone step.
#
# Progress stats are logged every 15 seconds. Interactive terminal runs also
# stream log lines and rclone output to stdout.
#
# Logs:  /tmp/push-remotely-save-<name>.log
# Lock:  /tmp/push-remotely-save-<name>.lock.d
...
LOG_FILE="/tmp/push-remotely-save-${NAME}.log"
LOCK_DIR="/tmp/push-remotely-save-${NAME}.lock.d"
...
# Refuse source values that look like rclone remotes. The source must be
# local so this job can never become a cloud-to-cloud sync.
if [[ "$SOURCE" == *:* ]]; then
    die "Source must be a local path, got: $SOURCE"
fi

if [[ ! -d "$SOURCE" ]]; then
    die "Source folder does not exist: $SOURCE"
fi
...
log "Starting push: $SOURCE -> ${REMOTE}:${BUCKET}"
if [[ -t 1 ]]; then
...
if [[ $RC -ne 0 ]]; then
    log "rclone copy exited with status $RC"
fi

log "Push complete (rclone rc=$RC)"
exit $RC
